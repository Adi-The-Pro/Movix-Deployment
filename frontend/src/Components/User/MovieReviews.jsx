...
//Taking Out The First Letter Of The Users Name
...
export default function MovieReviews() {
  ...
  const [reviews, setReviews] = useState([]); //Stroing all the reviews
  const [profileOwnersReview, setProfileOwnersReview] = useState(null); //storing the curr user's review
  ...
  //Taking out the movie id from the url param
  const { movieId } = useParams();
  //Fetching the logged in user's information
  const { authInfo } = useAuth();
  const profileId = authInfo.profile?.id;

  //Fetching review from the backend using movieId
  ...
  //Finding the logged in user's review out of all the reviews
    ...
    <div className="dark:bg-primary bg-white min-h-screen pb-10">
      <Container className="xl:px-0 px-2 py-8">
        <div className="flex justify-between items-center">
          ...
          {/*If the user is logged in then show this option*/}
          {profileId ? (
            <CustomButtonLink
        ...
        {/*Delete Review*/}
      <ConfirmModal visible={showConfirmModal} onCancel={hideConfirmModal} onConfirm={handleDeleteConfirm}
        busy={busy} title="Are you sure?"
      ...
      <EditRatingModal visible={showEditModal} initialState={selectedReview} 
        onSuccess={handleOnReviewUpdate} onClose={hideEditModal}
      />
