...
export default function PosterSelector({ name, accept, lable, selectedPoster, className, onChange,}){
  return (
    <div>
...
const PosterUI = ({ label, className }) => {
  return (
    <div className={commonPosterUI + " " + className}>
      <span className="dark:text-dark-subtle text-light-subtle text-white">{label}</span>
    </div>
  );
};
